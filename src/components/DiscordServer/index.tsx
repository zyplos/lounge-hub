...
const VoiceChannelIcon: React.FC = () => (
  // Typed as React.FC
  <svg
  ...
  >
    <path className={styles.voiceIconPath1} d="M0 0h60v60H0z" />
    <path
    ...
    <React.Fragment>
      <div className={styles.discordGrid}>
        ...
        <div className={styles.memberFlex}>
          ...
          <div className={styles.textContainer}>
            ...
            <p className={styles.statusText}>
              <span className={styles.statusTextBold}>{numberInVoice}</span> in
              channel.
            </p>
          </div>
        </div>
        ...
        {discordData.members.map((member) => (
          ...
          >
            <div
              className={`${styles.iconWrapperBase} ${getStatusClass(member.status)}`}
            >
              <Image
            ...
            <div className={styles.textContainer}>
              <p className={styles.usernameText}>
                ...
                {knownBots.includes(member.username) && (
                  <span
                    className={`badge-discord ${styles.botBadge}`}
              ...
              {member.status === "streaming" &&
                member.game && ( // Twitch game takes precedence for display
                  <p
                    className={`${styles.statusText} ${styles.statusTextStreaming} ${styles.statusTextBold}`}
                  >
                    Live on Twitch!
                  </p>
                )}
              {member.game && (
                <p
                  className={`${styles.statusText} ${member.status === "streaming" ? styles.statusTextStreaming : ""}`}
                >
                  <span className={styles.statusTextBold}>Playing</span>{" "}
                  {member.game.name}
                </p>
              )}
            </div>
