...
function DiscordServer() {
  ...
  if (error) {
    return (
      <FullBox useDims={false} usePadding> {/* Assuming FullBox should not use full dims here */}
        <p className="text-fullbox">Error getting Discord status.</p> {/* Using class from _common.css */}
        <pre>{JSON.stringify(error, null, 2)}</pre> {/* Changed false, 1 to null, 2 for better formatting */}
      </FullBox>
    );
  }
  if (!discordData) {
  ...
  // const voiceChannel = discordData.channels.find((channel) => channel.id === voiceChannelId); // Not used
  const numberInVoice = discordData.members.filter((m) => m.channel_id === voiceChannelId).length;
  // console.log(discordData); // Keep for debugging if necessary
