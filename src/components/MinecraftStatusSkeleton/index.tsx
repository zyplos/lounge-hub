...
interface MinecraftStatusSkeletonProps extends React.HTMLAttributes<HTMLDivElement> {
  // No custom props defined for this component
}

const MinecraftStatusSkeleton: React.FC<MinecraftStatusSkeletonProps> = (props) => {
  // Combine passed className with the default gridContainer class
  const combinedClassName = `${styles.gridContainer} ${props.className || ''}`.trim();

  return (
    <div {...props} className={combinedClassName}> {/* Spread props first, then override className */}
